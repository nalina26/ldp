...
class TDImageSlider extends Module {
    ...
    public function __construct() {
        ...
        $this->version = '2.0';//For themesdev Image slider
        $this->author = 'ThemesDeveloper';
           $this->bootstrap = true;
        $this->need_instance = 0;
        parent::__construct();
        $this->displayName = $this->l('ThemesDeveloper Home Image Slider');
        $this->description = $this->l('Home page Image Slider by ThemesDeveloper.');
    ...
    public function install() {
        ...
        if (parent::install() && $this->registerHook('displayTopColumn')) {
            /* Install tables */
            $response = tdimagesliderModel::createTables();
            return $response;
        }
        return false;
    }
    ...
    public function uninstall() {
        ...
        if (parent::uninstall()) {
            /* Deletes tables */
            $response = tdimagesliderModel::DropTables();
            return $response;
        }
        return false;
    }
    ...
    private function _insertSlider() {
        ...
        $moduledir=_PS_MODULE_DIR_.'tdimageslider/banner/';
        $moduleurl='modules/tdimageslider/banner/';
        ...
        if (Tools::isSubmit('tdSubmitValue')) {
            ...
            INSERT INTO `' . _DB_PREFIX_ . 'tdimageslider` (`image_link`,`active`,`position`,`new_page`, `id_shop`) 
        ...
        }elseif (Tools::isSubmit('changeStatus') AND Tools::getValue('id_tdimageslider')) {
            
            Db::getInstance()->Execute('
            UPDATE `' . _DB_PREFIX_ . 'tdimageslider`
            SET `active` = ' . (int) Tools::getValue('changeStatus') . '
            WHERE `id_tdimageslider` = ' .Tools::getValue('id_tdimageslider'));
...
clear:both;
		...
		<fieldset>
			<legend>' . $this->l('ThemesDeveloper Home Slider') . '</legend>      
...
';
   ...
   $this->context->controller->addJqueryUI('ui.sortable');
    ...
    padding-top: 15px;
        ...
        <legend>ThemesDeveloper Home Image Slider</legend>';
 ...
 $this->_html .= '<div class="panel" id="slides_' . $tdsliderdata['id_tdimageslider'] . '">
                                    <div class="row">
                                            <div class="col-lg-1">
                                            ...
                                            <div class="col-md-8">
                                                    <h4 class="pull-left">#' . $tdsliderdata['id_tdimageslider'] . ' - ' . $tdsliderdata['image_title'] . '<small>' . $tdsliderdata['image_link'] . '</small></h4>
                                                    <div class="btn-group-action pull-right">';
                                                        if ($tdsliderdata['active'] == 1) :                                     
                                                       $this->_html .= '<a title="Enabled" href="' . $currentIndex . '&configure=' . $this->name . '&token=' . Tools::getAdminTokenLite('AdminModules') . '&changeStatus=0&id_tdimageslider=' . (int) ($tdsliderdata['id_tdimageslider']) . '" class="btn btn-success"><i class="icon-check"></i> Enabled</a>';
                                                       else :
                                                       $this->_html .= '<a title="Disabled" href="' . $currentIndex . '&configure=' . $this->name . '&token=' . Tools::getAdminTokenLite('AdminModules') . '&changeStatus=1&id_tdimageslider=' . (int) ($tdsliderdata['id_tdimageslider']) . '" class="btn btn-danger"><i class="icon-remove"></i> Disabled</a>';
                                                          endif;
                                                            $this->_html .= '<a href="' . $currentIndex . '&configure=' . $this->name . '&token=' . Tools::getAdminTokenLite('AdminModules') . '&updateSlider&id_tdimageslider=' . (int) ($tdsliderdata['id_tdimageslider']) . '" class="btn btn-default">
                                                                    <i class="icon-edit"></i>
                                                                    Edit
                                                            </a>
                                                            <a href="' . $currentIndex . '&configure=' . $this->name . '&token=' . Tools::getAdminTokenLite('AdminModules') . '&deleteSlider&id_tdimageslider=' . (int) ($tdsliderdata['id_tdimageslider']) . '" class="btn btn-default">
