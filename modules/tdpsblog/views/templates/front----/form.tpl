<div class="comment-respond" id="respond">
        ...
        <h3 class="comment-reply-title" id="reply-title"><span>{l s='Leave a Comment' mod='tdpsblog'} </span>{if isset($replytocom) && $replytocom!=0}<small><a href="{$tdblogdetailslinks}{$alllinksalise}tdpost={$slide.id_tdpsblog}&replytocom=0" id="cancel-comment-reply-link" rel="nofollow">{l s='Cancel reply' mod='tdpsblog'}</a></small> {/if}</h3>
        ...
        <form class="comment-form" id="commentform" method="post" >
            ...
            {if isset($id_customer) && $id_customer!=''}</div>{/if}
                ...
                <p class="form-submit">
                   <input type="submit" class="button btn btn-default button-small" value="Post Comment" id="submit" name="submit" >
                   <input type="hidden" id="comment_post_ID" value="{$slide.id_tdpsblog}" name="comment_post_id">
