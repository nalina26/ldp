{if $totalcommentsbypost>0}
    <h3 id="comments-title">{$totalcommentsbypost} {l s='Response to' mod='tdpsblog'} <em>{$slide.tdpost_title|stripslashes|html_entity_decode}</em></h3>
            ...
            <li id="li-comment-{$tdblogpost.id_tdpsblog_comments}" class="comment byuser comment-author-admin bypostauthor even thread-even depth-1">
                <div id="comment-{$tdblogpost.id_tdpsblog_comments}">
                    ...
                    <div class="comment-meta">
                        <h5 class="author">{$tdblogpost.comment_author_name} -  <a  href="{$tdblogdetailslinks}{$alllinksalise}tdpost={$slide.id_tdpsblog}&replytocom={$tdblogpost.id_tdpsblog_comments}#respond" class="comment-reply-link">{l s='Reply' mod='tdpsblog'}</a></h5>	
                        <p class="date">
