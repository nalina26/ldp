...
private function getCatMobileOption($id_category, $id_lang = false, $id_shop = false) {
        ...
        $this->_mmenu .= '<option value="' . htmlentities($category_link) . '">' . $category->name . '</option>';
 ...
 private function makeMobileMenu($item) {
    ...
    private function FrontListMenu($parent = 0) {
        ...
        foreach ($td_menulinks as $td_menulink) {
            ...
            if($values[1]!='CAT'){
            if ($td_menulink['submenu']) {
                $this->_menu .= '<li id="menu-item-' . $td_menulink['id_tdmegamenu'] . '" class="parent ' . $selected . '">';
            } else {
                $this->_menu .= '<li id="menu-item-' . $td_menulink['id_tdmegamenu'] . '" class="' . $selected . '">';
            }
            }
            $this->_menu .=$this->makeMenu($td_menulink['menu_type']);
