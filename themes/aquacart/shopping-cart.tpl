...
{else}
							...
							<td rowspan="{3+$total_discounts_num+$use_show_taxes+$total_wrapping_taxes_num}" colspan="2" id="cart_voucher" class="cart_voucher">
								{if $voucherAllowed}
									...
									<form action="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" id="voucher">
										<fieldset>
											<h4>{l s='Vouchers'}</h4>
											<input type="text" class="discount_name form-control voucher-input" id="discount_name" name="discount_name" value="{if isset($discount_name) && $discount_name}{$discount_name}{/if}" />
											<input type="hidden" name="submitDiscount" />
											<button type="submit" name="submitAddDiscount" class="button btn btn-default button-small voucher-btn"><span>{l s='OK'}</span></button>
										</fieldset>
									</form>
									{if $displayVouchers}
	...
	{/if}
	<div id="shopping-cart-hook">{$HOOK_SHOPPING_CART}</div>
	<p class="cart_navigation clearfix">
		{if !$opc}
			<a
				...
				class="button btn btn-default standard-checkout button-medium shopping-cart-btn-right"
				title="{l s='Proceed to checkout'}">
				<span>{l s='Proceed to checkout'}<i class="icon-chevron-right right shopping-cart-btn-right-icon"></i></span>
			</a>
		{/if}
		<a
			...
			class="btn btn-default button button-medium shopping-cart-btn-left"
			title="{l s='Continue shopping'}">
			
                              <span><i class="icon-chevron-left shopping-cart-btn-left-icon"></i>{l s='Continue shopping'}</span>
