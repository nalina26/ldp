...
function WishlistCart(id, action, id_product, id_product_attribute, quantity)
{
	$.ajax({
		...
		success: function(data)
		{
			...
			{
                                                               showPopup();        
                    productImageSrc = jQuery('.main-image_'+id_product+' img').attr('src');                    
                    productImage = '<img width="72" src="'+productImageSrc+'" />';                    
                    productName = jQuery('.main-image_'+id_product+' img').attr('alt');                     
                    //cartHref = jQuery('#top-cart > a').attr('href');                    
                    popupHtml = productImage + '<em>'+productName+'</em> ' + successfullyaddedwishlist;
			       ...
			       if(action =='delete'){
                    ...
                    productName = jQuery('.main-image_'+id_product+' img').attr('alt');                     
                    //cartHref = jQuery('#top-cart > a').attr('href');                    
                    popupHtml = productImage + '<em>'+productName+'</em> ' + successfullydeletedwishlist;
                    popupWindow.find('.etheme-popup-content').css('backgroundImage','none').html(popupHtml);                    
                    jQuery('.cont-shop').one('click',function(){
                        hidePopup(popupOverlay,popupWindow);
                    });       
                    ...
                    productName = jQuery('.main-image_'+id_product+' img').attr('alt');                        
                    //cartHref = jQuery('#top-cart > a').attr('href');                    
                    popupHtml = productImage + '<em>'+productName+'</em> ' + needtologinforwishlist;
                    popupWindow.find('.etheme-popup-content').css('backgroundImage','none').html(popupHtml);                    
                    jQuery('.cont-shop').one('click',function(){
                        hidePopup(popupOverlay,popupWindow);
                    });       
