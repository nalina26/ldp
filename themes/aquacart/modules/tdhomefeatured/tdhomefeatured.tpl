...
<h1 class="general_heading">
 <span>{l s='Featured' mod='tdhomefeatured'}</span>
...
{if isset($product.new) && $product.new == 1}
    <span class="new-box">
        <span class="new-label">{l s='New!' mod='tdhomefeatured'}</span>
    </span>
{/if}
...
{elseif isset($product.on_sale) && $product.on_sale && isset($product.show_price) && $product.show_price && !$PS_CATALOG_MODE}
    <span class="sale-box">
        <span class="sale-label">{l s='Sale!' mod='tdhomefeatured'}</span>
    </span>
{/if}  
                            ...
                            {if ($product.id_product_attribute == 0 || (isset($add_prod_display) && ($add_prod_display == 1))) && $product.available_for_order && !isset($restricted_country_mode) && $product.minimal_quantity <= 1 && $product.customizable != 2 && !$PS_CATALOG_MODE}
                                    {if ($product.allow_oosp || $product.quantity > 0)}
                                            {if isset($static_token)}
                                                    <a class="button ajax_add_to_cart_button btn btn-default" href="{$link->getPageLink('cart',false, NULL, "add=1&amp;id_product={$product.id_product|intval}&amp;token={$static_token}", false)|escape:'html':'UTF-8'}" rel="nofollow" title="{l s='Add to cart' mod='tdhomefeatured'}" data-id-product="{$product.id_product|intval}">
                                                            <span>{l s='Add to cart' mod='tdhomefeatured'}</span>
                                                    </a>
                                            {else}
                                                    <a class="button ajax_add_to_cart_button btn btn-default" href="{$link->getPageLink('cart',false, NULL, 'add=1&amp;id_product={$product.id_product|intval}', false)|escape:'html':'UTF-8'}" rel="nofollow" title="{l s='Add to cart' mod='tdhomefeatured'}" data-id-product="{$product.id_product|intval}">
                                                            <span>{l s='Add to cart' mod='tdhomefeatured'}</span>
                                                    </a>
                                            {/if}
                                    {else}
...
<div class="welcome_text">
    {$themesdev.td_welcome_text|html_entity_decode}
</div>
