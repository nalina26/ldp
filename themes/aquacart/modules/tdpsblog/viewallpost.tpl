...
{if $tdblogpost}
        ...
        <article id="post-{$blogPost.id_tdpsblog}" class="article ">
            <div class="post-{$blogPost.id_tdpsblog} type-post status-publish format-standard hentry category-blanco-category category-demo-category instock">
               ...
               <a href="{tdpsblogClass::postlinks($blogPost.id_tdpsblog,$blogPost.link_rewrite)}" class="article-image"><img src="{$base_url}{$blogPost.image_url|html_entity_decode}" /></a>    
            	                                   
               <div class="entry-utility">
                    <span class="meta-prep meta-prep-author">{l s='Posted on' mod='tdpsblog'}</span> 
                    <a href="{tdpsblogClass::postlinks($blogPost.id_tdpsblog,$blogPost.link_rewrite)}" title="{$blogPost.tdpost_title|strip_tags}" rel="bookmark"><span class="entry-date">{$tdpostdate}</span></a> /
            				...
            				<p>{$blogPost.tdpost_content|html_entity_decode|strip_tags|truncate:302:''}<a href="{tdpsblogClass::postlinks($blogPost.id_tdpsblog,$blogPost.link_rewrite)}" class="more-link"><br /><span class="button active fl-r"><span>{l s='READ MORE' mod='tdpsblog'}</span></span><br /></a></p>
            				            			</div><!-- .entry-content -->
